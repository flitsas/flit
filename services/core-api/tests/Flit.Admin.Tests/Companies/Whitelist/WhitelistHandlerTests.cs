...
public sealed class WhitelistHandlerTests
{
    ...
    public async Task AC4_AddsEmails_AndWritesAuditPerInsertedEmail()
    {
        ...
        var rows = await verify.TenantWhitelistUsers.Where(w => w.TenantId == tenantId).ToListAsync();
    ...
    public async Task AC4_IsIdempotent_SkipsExistingEmails()
    {
        ...
        await using var verify = NewContext(db);
        (await verify.TenantWhitelistUsers.CountAsync(w => w.TenantId == tenantId)).Should().Be(2);
    ...
    public async Task AC5_InvalidEmail_Returns422_AndPersistsNothing()
    {
        ...
        await using var verify = NewContext(db);
        (await verify.TenantWhitelistUsers.CountAsync(w => w.TenantId == tenantId)).Should().Be(0);
    ...
    public async Task AC6_Get_ReturnsActiveWhitelist()
    {
        ...
        await using (var seed = NewContext(db))
        {
            seed.TenantWhitelistUsers.Add(new TenantWhitelistUser
            {
                Id = Guid.NewGuid(), TenantId = tenantId, Email = "a@co.com",
                AddedBy = AddedBy, CreatedAt = DateTimeOffset.UtcNow,
            });
            seed.TenantWhitelistUsers.Add(new TenantWhitelistUser
            {
                Id = Guid.NewGuid(), TenantId = tenantId, Email = "b@co.com",
        ...
        var result = await handler.HandleAsync(new GetWhitelistQuery { TenantId = tenantId });
    ...
    public async Task AC6_Get_ReturnsEmpty_WhenNoWhitelist()
    {
        ...
        var result = await handler.HandleAsync(new GetWhitelistQuery { TenantId = Guid.NewGuid() });
