...
- **Backend Agent**: `TramiteLifecycleService` único escritor de `status`; borra `PlateFlowStatus`, `PlateFlowStateMachine`, `PlateFlowCheckFields`, propiedad `ProcedureInstance.PlateFlowStatus`; `AssignPlateAsync/RevokePlateAsync/UpdatePlateAsync` transicionan vía lifecycle; `PermiteDecisionOt` desaparece (Approve/Reject ⇔ `status == entregado`); `AttachmentRules`, `ImprontaManualStampReadiness`, `PlateAssignmentEmailModelProjector`, `OtEstadoResolver`, `OtMetricsReadRepository`, `OtQueryRepository` leen `status`.
