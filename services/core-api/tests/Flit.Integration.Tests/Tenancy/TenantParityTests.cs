...
public sealed class TenantParityTests(PostgresDatabaseFixture fixture) : PostgresTestBase(fixture)
{
    ...
    public static IEnumerable<TheoryDataRow<Type, string>> ContratosDeRespuesta()
    {
        ...
        yield return new(typeof(InstanceSummaryDto), "Id ReferenceNumber Modalidad Estado Placa Vin VehiculoMarca VehiculoLinea CompradorNombre CompradorDocumento OrganismoTransito PasoActual TotalPasos CreatedAt TenantId CompaniaNombre DraftFinalizedAt IdentityValidationStatus SignaturePending CanSubmit Prioritario VendedorNombre VendedorDocumento SubsanacionActiva SubsanacionCount UltimoRechazoMotivo IsPaused PausedObservation Origin UpdatedAt GestorNombre Fuente FirmaVendedorEstado FirmaCompradorEstado ConsolidadoAttachmentId TipoNombre TipoCodigo PasoNombre TienePrenda TieneTransformacion RuntConfirmed RejectedFrom RevocationRequestStatus"); // RuntConfirmed: F12276 Confirmación RUNT; RejectedFrom: ADR-0059 origen del rechazo — ninguno es campo de jerarquía; F12565: sub-estado y decisión de revocatoria
