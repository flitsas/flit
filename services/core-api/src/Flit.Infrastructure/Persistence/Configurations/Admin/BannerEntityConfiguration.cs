...
/// <summary>
/// Mapeo EF Core de <c>admin.banners</c> (Feature #12236, ADR-0058 + ADR-0057). El DDL lo
/// gestiona la migracion SQL cruda (<c>107-HU12238-admin-banners.sql</c>): CHECK de vigencia,
/// indice parcial y los dos triggers. Entidad <c>ExcludeFromMigrations</c>, mismo patron que
/// <c>StandaloneDocumentEntityConfiguration</c>.
/// </summary>
internal sealed class BannerEntityConfiguration : IEntityTypeConfiguration<BannerEntity>
{
    public void Configure(EntityTypeBuilder<BannerEntity> builder)
    {
        ...
        builder.Property(x => x.ImageStoragePath)
            .HasColumnName("image_storage_path").HasMaxLength(200).IsRequired();
        builder.Property(x => x.ImageSha256)
            .HasColumnName("image_sha256").HasMaxLength(64).IsRequired();
