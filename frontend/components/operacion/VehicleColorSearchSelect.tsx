...
export type VehicleColorOption = { id: string; code: string; name: string };

/**
 * Selector de colores desde el catálogo persistido (`GET /tramites/vehicle-colors`).
 * Lista solo el nombre (descripción); persiste el `name` (valor FUR).
 */
export function VehicleColorSearchSelect({
  id,
  label,
  value,
  disabled = false,
  ...
  id: string;
  label: string;
  /** Descripción del color (valor efectivo del trámite). */
  value: string;
  disabled?: boolean;
  placeholder?: string;
  ...
  const [options, setOptions] = useState<VehicleColorOption[]>([]);
  ...
  const load = (term: string) => {
    ...
    setError(null);
    void tramitesClient
      .searchVehicleColors(term || undefined, 50, ac.signal)
      .then((items) => {
        if (ac.signal.aborted) return;
        // Si el valor actual no viene en la página, lo antepone para que se vea seleccionado.
        const cur = value.trim();
        const merged =
          cur && !items.some((i) => i.name.toUpperCase() === cur.toUpperCase())
            ? [{ id: 'current', code: '—', name: cur.toUpperCase() }, ...items]
            : items;
        setOptions(merged);
      })
      .catch((err: unknown) => {
  ...
  const pick = (opt: VehicleColorOption) => {
    onChange(opt.name);
    setOpen(false);
    setQuery('');
  };
