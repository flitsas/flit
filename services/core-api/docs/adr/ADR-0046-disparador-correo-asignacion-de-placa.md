...
4. **La variante Renting del composer es productiva y debe alinearse al canal.** Históricamente el composer documentaba `companyRegistered == "811011779"` y ADR-0046 §4 (versión original) resolvía la marca por NIT del tenant. Eso **divergía** de ADR-0045 y desacoplaba marca HTML del transporte. **Enmienda 2026-09-03:** la marca sigue `notification_channel` del tenant cliente (igual que aprobado/rechazado).
...
**4. Marca del cuerpo: por canal del tenant cliente (enmienda 2026-09-03).** Misma regla que ADR-0045 para `tramites.aprobado/rechazado`: `notification_channel = tenant_api` → ComposeRenting; `flit_smtp` (o default) → ComposeFlit. El transporte ya salía por `TenantChannelEmailRouter` según ese canal; la regla previa por NIT `811011779` desacoplaba marca y transporte y hacía parecer que correos SMTP FLIT “salían por API Renting”. Se retira la comparación contra NIT y el warning de discrepancia marca↔canal. El dueño de la política sigue siendo el **tenant cliente** (`ClientTenantId`), no el OT.
...
- ~~Un log nuevo de discrepancia marca↔canal (Decisión §4).~~ Retirado con la enmienda 2026-09-03 (marca = canal).

## ADRs relacionados

- **[ADR-0045]** — mecanismo de correo por cambio de estado del trámite. Este ADR lo **extiende** a un sub-estado que su outbox no cubre, reutilizando su patrón de cola + worker y su `TramiteNotificationRecipientResolver` sin modificarlo. No lo contradice ni lo supersede. **Enmienda 2026-09-03:** la resolución de la marca del cuerpo queda alineada a ADR-0045 (por canal del tenant cliente).
...
| R3 | ~~La marca Renting nunca acierta por comparar `tax_id` sin normalizar~~ **Mitigado / obsoleto (2026-09-03):** la marca ya no depende del NIT; sigue `notification_channel`. | — | — |
