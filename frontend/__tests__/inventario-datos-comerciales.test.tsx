 ...
 *
 * En Requisitos solo se captura el valor de venta (y el avalúo sugerido). Causal, tasa, derechos
 * y método de pago salieron de la UI: la causal viaja siempre como compraventa; los otros tres
 * se reenvían si ya estaban persistidos, para no borrar borradores viejos.
 */
...
describe('CommercialForm — inventario: valor de venta en Requisitos', () => {
  it('pide valor de venta y oculta causal, tasa, derechos y método de pago', async () => {
    await renderForm();
    ...
    expect(screen.queryByLabelText(/^Derechos/)).toBeNull();
    expect(screen.queryByLabelText('Método de pago')).toBeNull();
  });

  it('marca como obligatorio solo el valor de venta', async () => {
    await renderForm();

    expect(screen.getAllByLabelText('obligatorio')).toHaveLength(1);
  });

  it('el valor de venta se captura en pesos y viaja como entero, con causal compraventa', async () => {
  ...
  it('el pie del asistente guarda el valor y fuerza compraventa', async () => {
  ...
  it('si ya había tasa, derechos o método persistidos, el PUT los reenvía sin mostrarlos', async () => {
    mocks.getCommercial.mockResolvedValue({
      valorVenta: 10_000_000,
      causal: 'DONACION',
      tasaImpuesto: 1.5,
      derechos: 120000,
  ...
  it('sin valor de venta no guarda y lo dice en voz alta', async () => {
    await renderForm();
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(
      /Ingresa el valor de venta para continuar/,
    );
    expect(mocks.putCommercial).not.toHaveBeenCalled();
  });

  it('en solo lectura conserva el valor visible pero sin acciones', async () => {
    render(
...
describe('Paridad entre modalidades — los datos comerciales dentro de Requisitos', () => {
  ...
  it('traspaso: Requisitos pide valor de venta y avalúo, no liquidación ni causal', async () => {
