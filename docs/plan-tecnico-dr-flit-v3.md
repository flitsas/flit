...
| 2026-09-22 | Merge `develop` → rama | **6 conflictos resueltos** | `develop` avanzó mientras se trabajaba (AB#12711, HU12702, HU12699, a291ce55). Conviven `historialPlacaEnabled` (HU-C) y `canSearchValidaciones` (AB#12711) en `Shell`/`DrFlitAssistant`. El hook deja de leer el JWT a mano (ya lo hace `dr-flit-context.ts`). **Cambio semántico**: el rol OT se decide con `isOtUser` (rol `ot_admin` **o** tenant TRANSIT_OFFICE), porque desde AB#12711 la API del tenant rechaza a los organismos — caso nuevo en `dr-flit-context.test.ts`. Manual: se conservan las 6 secciones y 4 perfiles de v3, con «Gestor» y «Organismo de tránsito» desde `COPY.A06`/`COPY.A05` (HU12702); el texto «no cubre las consolas» se corrige a «las cubre según tu perfil». 526/529 en las suites tocadas (los 3 fallos son de `develop`: `Dashboard.test.tsx` espera «Total Trámites» y HU12700 dejó `COPY.E03` en «Total trámites»; `network-scope-analytics` no halla `biometria-solo-propia`). `tsc` limpio. |
