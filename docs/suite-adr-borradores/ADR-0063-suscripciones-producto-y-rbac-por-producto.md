# ADR-0063: Suscripción de productos por empresa y RBAC por producto
...
**Tags**: seguridad, RBAC, suscripciones, productos, SuperAdmin, AdminCompany, HU-10664  
...
- No existe producto ni suscripción. Hay tres booleans en `admin.tenant_operational_policies` (`TenantSettings.cs:135-149`) que **ningún endpoint aplica**: solo los lee `DashboardActiveModulesEndpoints.cs` para pintar tarjetas.
...
1. **No revertir HU #10664.** Se introduce **producto** (comercial) por encima de **módulo** (permisos).
2. Schema `platform` migrado por `core-api`:
   ...
   - `platform.tenant_product_subscriptions (tenant_id, product_code, status ACTIVE|SUSPENDED|CANCELLED, starts_at, ends_at, activated_by, notes)`, auditado en `admin.tenant_config_audit_logs`.
...
4. **Aplicación en tres puntos, todos obligatorios:**
   - Emisión del token (ADR-0062): sin suscripción activa o sin rol en el producto, no hay token para ese `aud`.
   - Cada API: policy `RequireProduct` que valida `aud` y estado de la suscripción, con caché en Redis invalidada por el evento `platform.subscription.changed`.
   - Launcher: `GET /api/v1/platform/me/apps` lista productos con suscripción activa y rol del usuario.
...
7. Migración: `tramites` activo para todas las empresas existentes; los booleans se convierten en suscripciones y se retiran tras un sprint.
...
- Sin vigencia, suspensión ni auditoría comercial.
...
### Opción 3: Schema `platform` con productos y suscripciones; `product_code` en módulos y roles — elegida

**Pros:**
- Separa lo comercial de los permisos.
- Un producto nuevo es un dato más su manifiesto.
- Aplicación en token, API y menú: cierra la brecha que motivó la HU #10664.
- Vigencias y suspensión sin borrar datos.

**Cons:**
- Tablas y pantallas nuevas; cambio en el constructor de roles.
- Los productos dependen de la caché de suscripciones.

**Esfuerzo:** M  
**Riesgos:** Caché desactualizada tras suspender; se mitiga con evento de invalidación y token de vida corta.

## Tradeoff aceptado

Se elige la **Opción 3**: un modelo nuevo a cambio de no mezclar lo comercial con los permisos y de aplicar la suscripción donde la HU #10664 mostró que faltaba.

## Consecuencias

### Lo que se gana

- SuperAdmin activa, suspende o vence productos con historial.
...
- **Backend Agent**: todo endpoint de producto lleva `RequireProduct` más su permiso; test de 403 con suscripción suspendida.
- **Frontend Agent**: ocultar en el menú no reemplaza el 403 de la API.
- **QA Agent**: matriz empresa con o sin producto × usuario con o sin rol × suscripción activa, suspendida o vencida × empresa hija.
- **Security Agent**: auditoría de cada activación, suspensión y asignación de rol.
