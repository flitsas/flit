...
/// <summary>
/// Mapea <c>tramites.plate_assignment_email_dispatches</c> (HU #11484).
/// DDL gestionado por SQL crudo (71): índices UNIQUE con <c>upper(plate)</c> y
/// <c>lower(recipient)</c>, CHECK y RLS no los genera el scaffolding de EF. Excluida del ModelSnapshot.
/// </summary>
internal sealed class PlateAssignmentEmailDispatchConfiguration
    : IEntityTypeConfiguration<PlateAssignmentEmailDispatch>
{
    public void Configure(EntityTypeBuilder<PlateAssignmentEmailDispatch> builder)
    {
        builder.ToTable(
        ...
        builder.Property(x => x.Plate).HasColumnName("plate").HasMaxLength(20).IsRequired();
